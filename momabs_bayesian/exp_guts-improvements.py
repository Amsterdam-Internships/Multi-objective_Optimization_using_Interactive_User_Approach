...
parser.add_argument('--print_logs', type=bool)
...
print('print logs', print_logs, args.print_logs)
cool = args.cool
print('cool', cool)
...
for num_seeds in range(1, 21):
    ...
    for utility_std in [0.1]:
        plt.suptitle('obj: {}, poly_n: {}, utl_std: {}, runs: {}, sigth: {}, bandit_var: {}, cool: {}'.format(num_objectives, poly_degree, utility_std, num_seeds, sigth, var_std, cool))
        for ground_truth in [False, True]:
            for sig in [True, False]:  # significance
                for add_virtual_comp in [False, True]:  # virtual comparisons

                    if ground_truth:
                        if not (sig and not add_virtual_comp and utility_std == 1):
                            continue
                    
                    print('num_seeds', num_seeds, 'truth', ground_truth, 'sig', sig, 'comp', add_virtual_comp)
                    regret_list = []
                    pull_count_list = []
                    num_questions_list = []
                    ...
                    if ground_truth:
                        ...
                        label = 'GUTS sig={} comp={}'.format(int(sig), int(add_virtual_comp))

                    plt.subplot(1, 2, 1)
                    plt.title(' ')
                    p_ts = plt.plot(range(num_iter), np.average(regret_list, axis=0), label=label)
                    plt.xlabel('time', fontsize=15)
                    plt.ylabel('regret', fontsize=15)
                    plt.legend(fontsize=10)

                    plt.subplot(1, 2, 2)
                    p_qs = plt.plot(range(num_iter), np.average(num_questions_list, axis=0), label=label)
                    plt.xlabel('time', fontsize=15)
                    plt.ylabel('questions', fontsize=15)

    # plot results and save figure
    plt.tight_layout()
    filepath_figure = get_filepath(num_objectives, poly_degree, sigth, var_std)
